...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    for part in ang hap neu sad; do
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    for x in ang neu sad hap train_clean_100 train_clean_360 train_other_500 dev-clean dev-other; do
...
lmexpname=exp/train_rnnlm_pytorch_lm_transformer_cosine_batchsize32_lr1e-4_layer16_unigram5000_ngpu4
...
if [ -z ${tag} ]; then
    expname=${train_set}_${backend}_train_pytorch_transformer.v1_aheads8_batch-bins15000000
    if ${do_delta}; then
